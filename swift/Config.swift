...
/// The data type the engine runs activations in. The prebuilt Metal accelerator's FLOAT16 path
/// scrambles exact-copy sequences — digits, dates — for Gemma 4 on iOS (google-ai-edge/LiteRT-LM
/// issue 2814); FLOAT32 on the GPU is the documented workaround, at the cost of memory and a
/// longer first shader compile. Nil leaves the engine's own default.
public enum ActivationDataType: Int32, Hashable, Sendable {
  case float32 = 0
  case float16 = 1
  case int16 = 2
  case int8 = 3
...
public struct EngineConfig: Hashable, Sendable {
  ...
  /// The activation data type, or nil for the engine's default. See `ActivationDataType`.
